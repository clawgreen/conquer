...
fn fn_spreadsheet_mine_revenue() {
    ...
    eprintln!("Mine: product={} (expected {}), rev_metal={} (expected {})",
...
fn fn_spreadsheet_mine_with_miner() {
    ...
    eprintln!("Mine+MINER: product={} (expected {})", metal_gain, expected_product);
    assert_eq!(metal_gain, expected_product, "MINER power should double metal");
}
...
fn fn_spreadsheet_mine_overpop() {
    ...
    eprintln!("Mine overpop: product={} (expected {})", metal_gain, expected);
...
fn fn_spreadsheet_city_revenue() {
    ...
    eprintln!("Capitol rev={} (expected {}), Town rev={} (expected {})",
    ...
    assert_eq!(spread.gold, expected_gold, "Total gold after spreadsheet mismatch");
}
...
fn fn_spreadsheet_city_architect() {
    ...
    eprintln!("Capitol+ARCHITECT rev={} (expected {})", spread.rev_cap, expected_cap);
    assert_eq!(spread.rev_cap, expected_cap, "ARCHITECT power should double city pop for tax");
}
...
fn fn_updcomod_food_consumption() {
    ...
    eprintln!("Food after consumption: {} (expected {})", gs.nations[ni].total_food, expected_food);
    assert_eq!(gs.nations[ni].total_food, expected_food, "Food consumption mismatch");
}
...
fn fn_updcomod_food_spoilage() {
    ...
    eprintln!("Food after spoilage: {} (expected {})", gs.nations[ni].total_food, expected);
    assert_eq!(gs.nations[ni].total_food, expected, "Food spoilage mismatch");
}
...
fn fn_updcomod_jewel_autobuy() {
    ...
    eprintln!("After jewel buy: gold={} (expected {}), jewels={} (expected {})",
        gs.nations[ni].treasury_gold, expected_gold,
        gs.nations[ni].jewels, expected_jewels);
    assert_eq!(gs.nations[ni].treasury_gold, expected_gold, "Gold after jewel buy mismatch");
    assert_eq!(gs.nations[ni].jewels, expected_jewels, "Jewels after buy mismatch");
}
...
fn fn_updmil_movement_reset() {
    ...
    eprintln!("DEFEND army move: {} (expected {}), MARCH army move: {} (expected {})",
        gs.nations[ni].armies[1].movement, expected_defend,
        gs.nations[ni].armies[2].movement, expected_march);
    assert_eq!(gs.nations[ni].armies[1].movement, expected_defend as u8, "DEFEND movement mismatch");
    assert_eq!(gs.nations[ni].armies[2].movement, expected_march as u8, "MARCH movement mismatch");
}
...
fn fn_updmil_maintenance_cost() {
    ...
    // C: tgold -= sold * maintenance[type%UTYPE]
    let maint_3 = UNIT_MAINTENANCE.get(3 % UTYPE as usize).copied().unwrap_or(50) as i64;
    ...
    eprintln!("Maintenance cost: {} (expected {} = 1000 * {})",
...
fn fn_move_cost_calculation() {
    ...
    assert!(gs.move_cost[4][4] < 0, "Water should be impassable (negative cost)");
...
fn fn_land_reachp_basic() {
    ...
    assert!(!r3, "2 sectors away should NOT be reachable with 1 move point");
...
fn fn_capture_unowned_sector() {
    ...
    assert_eq!(gs.sectors[3][3].owner, ni as u8, "Army should capture unowned sector");
...
fn fn_capture_requires_war() {
    ...
    assert_eq!(gs.sectors[3][3].owner, enemy as u8, "Should NOT capture without WAR");
    
    // Now declare war
    gs.nations[ni].diplomacy[enemy] = DiplomaticStatus::War as u8;
    update_capture(&mut gs, &mut rng);
    assert_eq!(gs.sectors[3][3].owner, ni as u8, "Should capture when at WAR");
}
...
fn fn_economy_roundtrip() {
    ...
    eprintln!("  Revenue: food={} metal={} cap={} other={}", 
        spread.rev_food, spread.rev_metal, spread.rev_cap, spread.rev_other);
    eprintln!("  Gold: {} -> {} (gain {})", 50000, spread.gold, spread.gold - 50000);
    ...
    let mut rng = ConquerRng::new(99);
    updmil(&mut gs, &mut rng);
    eprintln!("  After updmil: gold {} -> {} (cost {})", 
    ...
    updcomodities(&mut gs, &mut rng);
    eprintln!("  After updcomod: gold {} -> {}, food {} -> {}",
        gold_before_com, gs.nations[ni].treasury_gold,
        food_before, gs.nations[ni].total_food);
    
    // Verify nothing went catastrophically wrong
    eprintln!("  Final: gold={} food={} metal={} jewels={}",
        gs.nations[ni].treasury_gold, gs.nations[ni].total_food,
        gs.nations[ni].metals, gs.nations[ni].jewels);
    
    // Gold should be reasonable (not negative with these inputs)
    assert!(gs.nations[ni].treasury_gold > 0, "Gold went negative with moderate inputs");
}
